...
import CancellationDialog from "./core/CancellationDialog";
...
const Profile = () => {
  ...
  // Get subscription data from service
  const [subscriptionData, setSubscriptionData] = useState<any>(null);

  useEffect(() => {
    const subscription = SubscriptionService.getSubscriptionByUserId(mockUser.id);
    if (subscription) {
      const billingInfo = SubscriptionService.calculateBillingInfo(subscription);
      setSubscriptionData({
        ...subscription,
        billingInfo
      });
            ...
            <div className="pt-2 space-y-3">
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center space-x-2">
                  {subscriptionData?.status === 'active' && !subscriptionData?.isTrialSubscription && !subscriptionData?.cancelAtPeriodEnd && (
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {SubscriptionService.formatPrice(subscriptionData.monthly_price)}/month
                        {subscriptionData.offer_accepted && (
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">
                      {subscriptionData.billingInfo?.nextBillingDate && subscriptionData.billingInfo.nextBillingDate.toLocaleDateString('en-US', {
                        month: 'long',
                        day: 'numeric'
                      })}
                    </p>
                    <p className="text-sm text-gray-500">
                      {subscriptionData.billingInfo?.daysRemaining} days remaining
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
